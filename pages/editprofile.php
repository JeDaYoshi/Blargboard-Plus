...
if ($editUserMode || (HasPermission('user.edittitle') && (HasPermission('user.havetitle') || $user['posts'] >= Settings::get('customTitleThreshold'))))
	AddField('general', 'appearance', 'title', __('Status'), 'text', array('hint'=>__('Same thing as a Blargboard title.'), 'width'=>80, 'length'=>255));


if ($editUserMode || HasPermission('user.editavatars'))
{

	AddField('general', 'appearance', 'picture', __('Avatar'), 'displaypic', array('hint'=>__('Maximum size is 200x200 pixels.')));
}
...
AddField('general', 'presentation', 'timezone', __('Timezone offset'), 'timezone');
AddField('general', 'presentation', 'blocklayouts', __($blockall), 'checkbox', array($blockall));


// EDITPROFILE TAB -- PROFILE ------------------------------------------------
AddPage('profile', __('Profile'));

AddCategory('profile', 'profile', __('Profile Info'));

AddField('profile', 'profile', 'sex', __('Gender'), 'radiogroup', array('options'=>$sexes));
AddField('profile', 'profile', 'birthday', __('Birthday'), 'birthday');
AddField('profile', 'profile', 'location', __('Location'), 'text', array('width'=>24, 'length'=>60));
AddField('profile', 'profile', 'homepageurl', __('Homepage URL'), 'text', array('width'=>60, 'length'=>60));
AddField('profile', 'profile', 'homepagename', __('Homepage name'), 'text', array('width'=>60, 'length'=>60));

AddCategory('profile', 'about', __('About me'));

if ($editUserMode || HasPermission('user.editbio'))
	AddField('profile', 'about', 'bio', __('Bio'), 'textarea');


// EDITPROFILE TAB -- ACCOUNT -------------------------------------------------
AddPage('account', __('Account'));

AddCategory('account', 'confirm', __('Confirm password'));
...
AddCategory('account', 'settings', __('Account settings'));

if ($editUserMode)
	AddField('account', 'settings', 'name', __('Username'), 'text', array('width'=>24, 'length'=>20, 'callback' => 'HandleUsername'));
else
	AddField('account', 'settings', 'name', __('Username'), 'label', array('value'=>htmlspecialchars($user['name'])));

AddField('account', 'settings', 'password', __('Password'), 'password', array('callback'=>'HandlePassword'));

AddField('account', 'settings', 'email', __('Email'), 'email', array('width'=>24, 'length'=>60));
AddField('account', 'settings', 'showemail', __('Make email address public'), 'checkbox');


if ($editUserMode)
{
	AddCategory('account', 'admin', __('Administration'));
...
{
	$pltext = $pltype ? __('Layout') : __('Signature');
